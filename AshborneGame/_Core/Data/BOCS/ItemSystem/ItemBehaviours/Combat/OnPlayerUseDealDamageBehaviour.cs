...
namespace AshborneGame._Core.Data.BOCS.ItemSystem.ItemBehaviours.Combat
{
    ...
    {
        public override string SaveId => "onPlayerUseDealDamage";
        ...
        public override void LoadSaveData(BehaviourSaveData data, SaveLoadContext context)
        {
            ...
            {
                throw new InvalidDataException("OnPlayerUseDealDamageBehaviour save data is missing state.");
            }
            SaveData save = JsonSerializer.Deserialize<SaveData>(data.State.Value) ?? throw new InvalidDataException("Failed to deserialise OnPlayerUseDealDamageBehaviour save data.");
