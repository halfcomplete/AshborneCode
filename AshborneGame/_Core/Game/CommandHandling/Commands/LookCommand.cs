...
namespace AshborneGame._Core.Game.CommandHandling.Commands
{
    public class LookCommand : ICommand
    {
        ...
        {
            IOService.Output.WriteLine(player.CurrentSublocation?.GetDescription(player, GameContext.GameState) ?? player.CurrentLocation.GetLookDescription(player, GameContext.GameState));
            return true;
        }
    }
}
