...
public partial class Home : ComponentBase, IDisposable
{
    ...
    private async Task AnimateBlurOverlayAsync(float targetOpacity, float durationSecs, float fadeBackDurationSecs, float waitSecs)
    {
        try
        {
            Console.WriteLine($"[DEBUG] AnimateBlurOverlayAsync called with targetOpacity={targetOpacity}, durationSecs={durationSecs}, fadeBackDurationSecs={fadeBackDurationSecs}, waitSecs={waitSecs}");
            ...
            // Create a reference to this component so JavaScript can call back
            var dotNetReference = DotNetObjectReference.Create(this);

            // Call JavaScript function to animate the blur overlay
            await JS.InvokeVoidAsync("animateBlurOverlay", targetOpacity, durationSecs, fadeBackDurationSecs, waitSecs, dotNetReference);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[ERROR] AnimateBlurOverlayAsync failed: {ex.Message}");
        }
    }

    [JSInvokable]
    public void DisableBlurOverlay()
    {
        Console.WriteLine("[DEBUG] DisableBlurOverlay called from JavaScript");
    ...
    private bool VerifyBlurAnimationMarker(string line, out float targetOpacity, out float durationSecs, out float fadeBackDurationSecs, out float waitSecs)
    {
        targetOpacity = 0f;
        durationSecs = 0f;
        fadeBackDurationSecs = 0f;
        waitSecs = 0f;
        IOService.Output.DisplayDebugMessage("VerifyBlurAnimationMarker: Detected blur animation marker in line: " + line);
        var match = OutputConstants.BlurAnimationRegex.Match(line);
        if (!(match.Success && match.Groups.Count >= 5))
        {
            return false;
        }
        IOService.Output.DisplayDebugMessage("VerifyBlurAnimationMarker: Blur animation regex matched successfully.");
        if (float.TryParse(match.Groups[1].Value, out targetOpacity) &&
            float.TryParse(match.Groups[2].Value, out durationSecs) &&
            float.TryParse(match.Groups[3].Value, out fadeBackDurationSecs) &&
            float.TryParse(match.Groups[4].Value, out waitSecs))
        {
            IOService.Output.DisplayDebugMessage($"VerifyBlurAnimationMarker: Detected blur animation marker (targetOpacity={targetOpacity}, durationSecs={durationSecs}, fadeBackDurationSecs={fadeBackDurationSecs}, waitSecs={waitSecs})");
            return true;
        }
        return false;
    }
    ...
    /// <summary>
    /// Verifies the pause marker in a line and extracts the ms value.
    /// </summary>
    /// <remarks>
    /// The line should end with the pause marker, and the ms value should be at the start of the line (e.g. "500__PAUSE__").
    /// Displays a debug message if we fail to parse the ms value.
    /// </remarks>
    /// <param name="line">The line to verify.</param>
    /// <param name="ms">The parsed ms value.</param>
    /// <returns>True if the line is a pause marker and the ms value was parsed successfully, otherwise false.</returns>
    private bool VerifyPauseMarker(string line, out int ms)
    {
        dialogueChoices.Clear();
        var msStr = line.Substring(0, line.IndexOf(OutputConstants.DialoguePauseMarker));
        if (int.TryParse(msStr, out ms))
        {
            return true;
        }
        else
        {
            IOService.Output.DisplayDebugMessage($"VerifyPauseMarker failed to parse ms from the line: '{line}'");
            return false;
        }
    }
    ...
    private async Task HandleTypewriterEffect(string line)
    {
        ...
        if (OutputConstants.SpeakerTagRegex.IsMatch(line)) return;
        ...
        const int batchSize = 2; // Update UI every 2 characters for smoother performance
        ...
        foreach (var token in tokens)
        {
            if (token.isTag)
            {
                // Append tags immediately (they don't consume characterSpeed slots)
                typedBuilder.Append(token.text);
                // Only update if we have pending characters
            ...
            // Type text chunk character by character
            for (int ci = 0; ci < token.text.Length; ci++)
            {
                char letter = token.text[ci];
                // Append encoded character
                typedBuilder.Append(System.Net.WebUtility.HtmlEncode(letter.ToString()));
                charsSinceLastUpdate++;

                // Determine isEnd (approx): consider space or end-of-chunk
                bool isEnd = (ci == token.text.Length - 1) || (ci + 1 < token.text.Length && token.text[ci + 1] == ' ');

                int currentSpeed = ms;
                if (characterSpeeds != null && characterSpeeds.Count > globalCharIndex)
                    currentSpeed = characterSpeeds[globalCharIndex];

                // Batch update: only update UI every batchSize characters or at end
                bool shouldUpdate = charsSinceLastUpdate >= batchSize || isEnd;
                if (shouldUpdate)
                {
                    var separator = string.Empty;
                    if (!string.IsNullOrEmpty(existing) && !existing.EndsWith("<br />"))
...
#if DEBUG
                int delay = await CharacterOutputDelayCalculator.CalculateDebugDelayAsync(letter, currentSpeed, isEnd);
#else
                int delay = await CharacterOutputDelayCalculator.CalculateDelayAsync(letter, currentSpeed, isEnd);
#endif
    ...
    /// <summary>
    /// Parses the ms value from the start of a line with a typewriter marker. If parsing fails, returns the default type speed value.
    /// </summary>
    /// <remarks>Displays a debug message showing the parsed ms.</remarks>
    /// <returns>The ms value parsed.</returns>
    private int ParseMsFromTypewriterStartMarker(string line)
    {
        ...
        IOService.Output.DisplayDebugMessage($"Typewriter effect with delay: {ms} ms");
        return ms;
    }

    /// <summary>
    /// Parses the content between the two typewriter markers in a line.
    /// </summary>
    /// <param name="line">The line to parse the content for.</param>
    /// <returns>The content of the line.</returns>
    private string ParseContentFromTypewriterMarkerPair(string line)
    {
        ...
        {
            // Markers not found, throw an invalid operation exception
            throw new InvalidOperationException($"Typewriter markers not found in line: {line}");
        }
        string message = line.Substring(startIndex, endIndex - startIndex);
        IOService.Output.DisplayDebugMessage($"Typewriter message: {message}");
        return message;
    }
    ...
    private static string ParseSpeakerTags(string input)
    {
        ...
        // Split into lines while preserving newline tokens
        var parts = Regex.Split(input, "(\\r?\\n)");
        IOService.Output.DisplayDebugMessage($"[DEBUG] ParseSpeakerTags: Split input into {parts.Length} parts: {string.Join(", ", parts)}");
        for (int i = 0; i < parts.Length; i++)
        {
            var line = parts[i];
            IOService.Output.DisplayDebugMessage($"[DEBUG] ParseSpeakerTags: Processing part {i}: '{line}'");
            if (string.IsNullOrWhiteSpace(line))
                continue;
            IOService.Output.DisplayDebugMessage($"[DEBUG] ParseSpeakerTags: Checking for speaker tag in line: '{line}'");
            var match = OutputConstants.SpeakerTagRegex.Match(line);
            if (!match.Success)
                continue;

            IOService.Output.DisplayDebugMessage($"[DEBUG] ParseSpeakerTags: Found speaker tag in line: '{line}'");
    ...
    private static string ParseColours(string input)
    {
        if (string.IsNullOrEmpty(input)) return input;

        // Strict first
        input = OutputConstants.ColourTagRegexStrict.Replace(input, m =>
        {
        ...
        // Then lenient (fix missing slash on the closing tag)
        input = OutputConstants.ColourTagRegexLenient.Replace(input, m =>
        {
            var hex = m.Groups[1].Value;
            var inner = m.Groups[2].Value;
            return $"<span style=\"color:#{hex};\">{inner}</span>";
    ...
    private static string ParseSFX(string input)
    {
        if (string.IsNullOrEmpty(input)) return input;

        // Replace SFX markers with spans using SFXFormatter service
        return SFXFormatter.ConvertSFXToSpans(input);
    }

    private string RenderGameText(string line)
    {
        if (string.IsNullOrEmpty(line))
            return string.Empty;

        line = ParseSpeakerTags(line);
        // First: colour tags -> <span style="color:#...">
        line = ParseColours(line);
        IOService.Output.DisplayDebugMessage($"RenderGameText: After ParseColours: '{line}'");

        // Then: <i>...</i> -> <span class='game-italic'>...</span>
        line = ParseItalics(line);

        // Finally: SFX tags (after all other HTML is in place)
        line = ParseSFX(line);

        return line;
    }

    private string ParseItalics(string line)
    {
        ...
        {
            int start = line.IndexOf(OutputConstants.InlineItalicStartMarker, idx);
            if (start == -1)
            {
                sb.Append(line.Substring(idx).Replace("\n", "<br />"));
                break;
            }

            // Append text before <i>
            sb.Append(line.Substring(idx, start - idx).Replace("\n", "<br />"));

            int end = line.IndexOf(OutputConstants.InlineItalicEndMarker, start);
            if (end == -1)
            {
                // Unterminated <i>-treat the remainder as plain text
                sb.Append(line.Substring(start).Replace("\n", "<br />"));
                break;
            }

            int contentStart = start + OutputConstants.InlineItalicStartMarker.Length;
            string italicContent = line.Substring(contentStart, end - contentStart);

            sb.Append($"<span class='game-italic'>{italicContent.Replace("\n", "<br />")}</span>");
